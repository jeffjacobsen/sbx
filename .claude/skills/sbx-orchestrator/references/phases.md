...
Two layers. Which layer a file belongs to is decided by one question:
**what credential does it need?**

| | OUT-sandbox (orchestration) | IN-sandbox (execution) |
| --- | --- | --- |
| Lives in | the sbx install: `just/`, `lib/` | the target repo's checkout, at `app/` on the VM |
| Runs on | your machine | the VM |
| Credential | exe.dev account + `OPENROUTER_PROVISIONING_KEY` | the disposable runtime key only |
| Entry point | `sbx mount` / `lifecycle execute` / `lifecycle teardown` | whatever `kickoff.default` names in that repo's `sandbox.yaml` |
...
`OPENROUTER_PROVISIONING_KEY` never leave the host, so a sandbox that tries `sbx mount` gets
...
| **execute** `RUN_ID "PROMPT"` | host-triggered, the manifest's `kickoff.default` runs inside, detached | `vm_name` in the record; a passed SETUP; a `kickoff.default` in the manifest | the remote shell returns a numeric PID (non-numeric = the remote printed an error where the PID should be) | `pid` |
| **observe** `RUN_ID` | host + VM — starts the declared services, retargets the proxy | `vm_name`; every service's declared `requires:` present on the VM (a service that needs a file work has not produced yet exits on the spot) | app returns **200 anonymously**; obs port is reachable (anything that is not `000` and not `5xx`) | `ports` |
...
| Recipe | Shape | Notes |
| --- | --- | --- |
| `sbx run cmd RUN_ID '<cmd>'` | synchronous, prints output, `cd app &&` first | generic escape hatch. `{{CMD}}` is interpolated verbatim so your pipes/globs/quotes reach the remote shell intact |
| `sbx manage harvest RUN_ID` | synchronous, reads the box, writes `refs/sandbox/<run-id>` | bundles `<commit_sha>..sbx/<run-id>` on the VM, copies it to `~/.local/state/sbx/runs/<id>.bundle`, verifies, fetches. **Non-destructive and idempotent** — run it the moment a run commits, don't wait for teardown. Writes nothing to the record: the bundle's existence *is* the record, same as `.agent-started`. A run filled before run branches existed gets `sbx/<run-id>` created at HEAD, which touches no files |
| `sbx run agent RUN_ID "PROMPT"` | synchronous Claude Code turn inside the box | reads `session_id`; first call uses `--session-id`, every later one `--resume`. "Has it started" is a **host sentinel** `~/.local/state/sbx/runs/<id>.agent-started`, touched only after a successful turn — the record schema is closed and has nowhere to put a boolean, and probing the VM's session layout would depend on an unverified implementation detail. Runs on exe.dev's key-free gateway (`ANTHROPIC_BASE_URL=https://llm.int.exe.xyz`, `ANTHROPIC_API_KEY=implicit`), **not** OpenRouter |

And the fleet-wide backstop, which takes no run id:

`sbx manage reap [--yes]` — dry run by default. Joins the OpenRouter key list × `ssh exe.dev ls --json`
...
## Readback: `just/manage/list.just`
...
| --- | --- | --- |
| `sbx manage list` | RUN, STATE (`open`/`closed` from `closed_at`), VM (`up`/`gone`/`?`), SPEND, CREATED | `run_record.py list` + **one** `ssh exe.dev ls --json` call for the whole table. `?` is deliberately distinct from `gone` — a failed control-plane lookup must never render as "every VM is destroyed" |

Formatting happens in `lib/runs_table.py`, not a shell read-loop, for two reasons:
...
| Level | Reads | Belongs |
| --- | --- | --- |
| **Factory** | `.claude/skills/sssf/SKILL.md` | **inside** a sandbox. Running it on the host runs the factory on your laptop, which is the one thing this system exists to prevent |
| **Orchestration** | this skill, `.claude/skills/sbx-orchestrator/SKILL.md` | the **host**, and only the host — it needs the exe.dev account and the provisioning key, neither of which reaches a sandbox |

Booting the orchestration level needs no recipe: run `claude` in the sbx repo and the skill is
discovered from `.claude/skills/`. From another repo, point it at
`$SBX_HOME/.claude/skills/sbx-orchestrator/SKILL.md`. There was an `sbx orch cc` / `orch pi` pair
that did exactly this, carried over from sandbox-factory; it was deleted rather than repaired,
because a hardcoded skill path in a recipe is a second name to keep in sync and the wrapper saved
nothing over typing `claude`.

`sbx run agent RUN_ID "<prompt>"` talks to the factory orchestrator already running *inside* a
mounted sandbox, over ssh, in a resumable session — so you can keep steering without ssh-ing in
yourself.

---

## The gate assertions (SETUP)

**Only A is sbx's.** It is the builtin, and it runs for every repo. B–E are *declared*, in the
mounted repo's own `health:` block — they used to be hardcoded here, and they moved out because they
are claims about the payload ("an SSSF factory driven by pi over OpenRouter"), not about sandboxes.
They are spelled out below because sandbox-factory is the worked example and its five-gate shape is
what most runs you see will report; a repo that declares no `health:` block still gates on A.
...
| **C** | every model in the **active** config answers a ping | Catches model-id drift. The roster path arrives as `$1` — SETUP forwards its `CONFIG` argument to every `script` assertion, because `ssh host 'cmd'` carries no environment and an env-var route always fell through to the default. The `openrouter/` provider prefix is stripped. Reasoning models answer with `content: null` and the text under `.reasoning`, so the check accepts either. |
...
inspect the box, exits non-zero, and says `sbx lifecycle teardown <run-id>` is your call. An earlier draft
...
| 2. artifacts | skipped if no VM. Paths come from the manifest's `artifacts:`, defaulting to `run.log` — and falling back to that default on ANY manifest failure, because nothing on this path may be able to block the revoke. `ls -d` filters to what exists, so a run that died before writing one of them is not an error. Empty result removes the dir rather than implying something was pulled |
| 3. harvest | delegates to `sbx manage harvest`, **default on**, so nobody loses a run's commits by forgetting. Skipped if no VM, or deliberately with `--no-harvest` when you already harvested. A failure **ABORTS the teardown before revoke and destroy** — destroying a VM whose commits were never pulled is the one irreversible mistake this file can make |
...
```
sbx mount RUN_ID [--limit N]    # create -> fill -> setup -> observe
```
...
`sbx lifecycle create` accepts either a task description (it gets slugified and suffixed with
...
Everything host-only lives under `just/` or `lib/` — the readback recipes,
