...
Implementation: `lib/run_record.py`. Stdlib only, on purpose — it runs on the host
...
| 5 | `limit` | create | operator, teardown math | float | Dollars on the OpenRouter key. Default `50.00`, per-run via `sbx lifecycle create <id> --limit N` |
...
```bash
RUN_VM=$(lib/run_record.py get my-run vm_name)
```

An unset field prints an **empty line**, so emptiness — not exit status — is the real check.
Every phase does `[ -n "$VM" ] || { echo "... run 'sbx lifecycle create' first"; exit 1; }`. Only `dict`
...
| `<run-id>.agent-started` | `sbx run agent`, after a **successful** turn | the "has this session started" bit that decides `--session-id` vs `--resume`. It is a host sentinel because the schema is closed and has nowhere to put a boolean, and probing the VM for Claude Code's on-disk session layout would make an unverified implementation detail the source of truth. Touched only after success — a first call that died never created a session, and `--resume` on a session that does not exist would fail forever |
